...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        file_layout.addWidget(self.undo_btn)

        # Preserve erasures checkbox
        self.preserve_eraser_checkbox = QCheckBox("Preserve Erased Regions")
        self.preserve_eraser_checkbox.setChecked(True)
        file_layout.addWidget(self.preserve_eraser_checkbox)
        ...
        morph_group = QGroupBox("Morphological Closing")
        morph_layout = QGridLayout()

        morph_explanation = QLabel(
            "Morphological Closing:\nControls the size of the kernel used for closing.\n"
            "Larger values help fill gaps in the shapes."
        )
        morph_explanation.setWordWrap(True)
        morph_layout.addWidget(morph_explanation, 0, 0)
    ...
    def update_edge_detection(self):
        if self.original_image is not None:
            # Store current edge image for undo if it exists
            if self.edge_image is not None and not self.showing_original:
                self.add_to_history(self.edge_image.copy())
            ...
            # Morphological closing to fill holes - use slider value
            kernel_size = self.morph_size.value()
            ...
            # Apply saved eraser actions if requested
            if self.preserve_eraser_checkbox.isChecked() and self.erased_regions:
                self.apply_erased_regions()
    ...
    def apply_eraser(self, current_pos):
        ...
        # Store erased region if preserve checkbox is checked
        if self.preserve_eraser_checkbox.isChecked():
            self.erased_regions.append((current_pos, radius))
    ...
    def apply_erased_regions(self):
        """Apply all saved eraser actions to the current edge image"""
        if not self.edge_image is None and self.erased_regions:
            # Convert to grayscale for processing
            gray = cv2.cvtColor(self.edge_image, cv2.COLOR_RGB2GRAY)

            # Apply each erased region
            for pos, radius in self.erased_regions:
                cv2.circle(gray, pos, radius, 255, -1)

            # Convert back to RGB
            self.edge_image = cv2.cvtColor(gray, cv2.COLOR_GRAY2RGB)

    def add_to_history(self, image):
        """Add current image state to history for undo"""
    ...
    def undo(self):
        ...
        if self.history_index > 0 and self.history:
            ...
            self.history_index -= 1
            self.edge_image = self.history[self.history_index].copy()
            ...
            # Remove the last eraser action if appropriate
            if self.preserve_eraser_checkbox.isChecked() and self.erased_regions:
                self.erased_regions.pop()
